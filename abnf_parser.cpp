...
bool abnf_element::run(str_const_iterator& it, const str_const_iterator& end)
{
    assert(this->element.get());

    str_const_iterator jt = it;
    bool m = (this->element->run(jt, end) || this->is_option);
...
bool abnf_vals::generate(str_const_iterator& it, const str_const_iterator& end)
{
    ...
    else if(jt != end && *jt == '%')
    {
        ...
        if(parser.run(jt, end))
        {
            int num = -1;
            if(!parser.get_rule("bin-val")->matched_pattern.empty())
            {
                assert(false); // bin not implemented
                parser.add_rule("STR = 1*BIT");
                num = 0;
            }
            else if(!parser.get_rule("dec-val")->matched_pattern.empty())
            {
                parser.add_rule("STR = 1*DIGIT");
                num = 1;
            }
            else if(!parser.get_rule("hex-val")->matched_pattern.empty())
            {
            ...
            if(!parser.get_rule("DOT")->matched_pattern.empty())
            {
                std::string num_val = parser.get_rule("num-val")->matched_pattern;
                ...
                while(parser.get_rule("iterator")->run(kt, num_val.end()))
                {
                    unsigned int val;
                    std::stringstream sts;

                    if(num == 1)
                        sts << std::dec << parser.get_rule("STR")->matched_pattern;
                    else if(num == 2)
                        sts << std::hex << parser.get_rule("STR")->matched_pattern;
            ...
            else if(!parser.get_rule("DASH")->matched_pattern.empty())
            {
                std::string num_val = parser.get_rule("num-val")->matched_pattern;
                str_const_iterator kt = num_val.begin();
            ...
            else
            {
                ...
                unsigned int val;
                if(num == 1)
                    sts << std::dec << parser.get_rule("DECSTR")->matched_pattern;
                else if(num == 2)
                    sts << std::hex << parser.get_rule("HEXSTR")->matched_pattern;
                sts >> val;
    ...
    else if(jt != end && *jt == '<')
    {
        ...
        if(parser.run(jt, end))
        {
            this->type = CHAR_VAL;
            this->sensitive = false;
            this->char_val = parser.get_rule("STR")->matched_pattern;
            EXPR_MATCHED(true);
            return true;
        }
    }
...
bool abnf_vals::run(str_const_iterator& it, const str_const_iterator& end)
{
    str_const_iterator jt = it;
...
bool abnf_rulename::generate_rulename(str_const_iterator& it, const str_const_iterator& end)
{
...
bool abnf_rulename::run(str_const_iterator& it, const str_const_iterator& end)
{
    assert(this->rule);

    str_const_iterator jt = it;
    bool m = this->rule->run(jt, end);
...
bool abnf_repetition::run(str_const_iterator& it, const str_const_iterator& end)
{
    str_const_iterator jt = it;

    if(!this->has_repeat)
    {
        bool m = this->element.run(jt, end);
        EXPR_MATCHED(m);
        return m;
    }
    else
    {
        ...
        int count = 0;
        while(this->element.run(jt, end)) count++;
...
bool abnf_concatenation::run(str_const_iterator& it, const str_const_iterator& end)
{
    str_const_iterator jt = it;

    if(!this->left.run(jt, end))
        return false;
    else
    {
...
bool abnf_alternation::run(str_const_iterator& it, const str_const_iterator& end)
{
    str_const_iterator jt = it;
    if(this->left.run(jt, end))
    {
...
abnf_rule::abnf_rule(abnf_parser& parser) : 
    parser(parser), 
    alternation(parser),
    generated(false), matched(false)
{
}
...
bool abnf_rule::generate(str_const_iterator& it, const str_const_iterator& end)
{
    ...
    if(!rulename.generate_rulename(jt, end))
        return false;
    this->rulename = rulename.rulename;
...
bool abnf_rule::run(str_const_iterator& it, const str_const_iterator& end)
{
    assert(this->generated);
    // TODO: incremental
    str_const_iterator jt = it;
    this->matched = this->alternation.run(jt, end);

    if(this->matched)
        this->matched_pattern.assign(it, jt);

    EXPR_MATCHED(this->matched);
    return this->matched;
}

abnf_parser::abnf_parser() : entry(*this)
{
}

bool abnf_parser::add_rule(std::string syntax)
{
    syntax += "\r\n";
...
bool abnf_parser::run(const std::string& input)
{
    return this->run(input.begin(), input.end());
}

bool abnf_parser::run(str_const_iterator& it, const str_const_iterator& end)
{
    return this->entry.run(it, end);
}
